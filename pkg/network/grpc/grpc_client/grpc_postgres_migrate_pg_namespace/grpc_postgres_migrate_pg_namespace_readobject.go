...
func (crud Crud_GRPC) ReadObject(m *object_postgres_migrate_pg_namespace.ObjectPostgresMigratePgNamespace) error {
	var err error
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PostgresMigratePgNamespace_ReadObject(Request)
	} else {
		Response, err = grpc_client_func.Client.PostgresMigratePgNamespace_ReadObject(ctx, Request)
	}
	if err != nil {
