...
func (crud Crud_GRPC) ReadObject(m *object_postgres_migrate_pg_sequence.ObjectPostgresMigratePgSequence) error {
	var err error
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PostgresMigratePgSequence_ReadObject(Request)
	} else {
		Response, err = grpc_client_func.Client.PostgresMigratePgSequence_ReadObject(ctx, Request)
	}
	if err != nil {
