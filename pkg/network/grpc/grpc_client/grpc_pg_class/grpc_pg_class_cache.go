...
func (crud Crud_GRPC) ReadFromCache(Oid int64, VersionID int64) (pg_class.PgClass, error) {
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgClass_ReadFromCache(Request)
	} else {
		Response, err = grpc_client_func.Client.PgClass_ReadFromCache(ctx, Request)
	}
	if err != nil {
