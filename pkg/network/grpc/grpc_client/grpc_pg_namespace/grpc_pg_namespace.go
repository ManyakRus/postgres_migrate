...
func (crud Crud_GRPC) GetVersionModel() uint32 {
	...
	if VersionModel == 0 {
		VersionModel = pg_namespace.PgNamespace{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_GRPC) Read(m *pg_namespace.PgNamespace) error {
	var err error
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgNamespace_Read(Request)
	} else {
		Response, err = grpc_client_func.Client.PgNamespace_Read(ctx, Request)
	}
	if err != nil {
...
func (crud Crud_GRPC) Create(m *pg_namespace.PgNamespace) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgNamespace_Create(Request)
	} else {
		Response, err = grpc_client_func.Client.PgNamespace_Create(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
...
func (crud Crud_GRPC) Update(m *pg_namespace.PgNamespace) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgNamespace_Update(Request)
	} else {
		Response, err = grpc_client_func.Client.PgNamespace_Update(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
...
func (crud Crud_GRPC) Save(m *pg_namespace.PgNamespace) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgNamespace_Save(Request)
	} else {
		Response, err = grpc_client_func.Client.PgNamespace_Save(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
