...
func (crud Crud_GRPC) ReadFromCache(Oid int64, VersionID int64) (pg_namespace.PgNamespace, error) {
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgNamespace_ReadFromCache(Request)
	} else {
		Response, err = grpc_client_func.Client.PgNamespace_ReadFromCache(ctx, Request)
	}
	if err != nil {
