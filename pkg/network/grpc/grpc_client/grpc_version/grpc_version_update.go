...
func (crud Crud_GRPC) UpdateManyFields(m *version.Version, MassNeedUpdateFields []string) error {
	var err error
	...
	if grpc_nrpc.NeedNRPC == true {
		//_, err = nrpc_client.Client.Version_UpdateManyFields(Request)
	} else {
		_, err = grpc_client_func.Client.Version_UpdateManyFields(ctx, Request)
	}
	if err != nil {
...
func (crud Crud_GRPC) Update_Description(m *version.Version) error {
	var err error
	...
	// запрос
	if grpc_nrpc.NeedNRPC == true {
		//_, err = nrpc_client.Client.Version_Update_Description(Request)
	} else {
		_, err = grpc_client_func.Client.Version_Update_Description(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
...
func (crud Crud_GRPC) Update_Name(m *version.Version) error {
	var err error
	...
	// запрос
	if grpc_nrpc.NeedNRPC == true {
		//_, err = nrpc_client.Client.Version_Update_Name(Request)
	} else {
		_, err = grpc_client_func.Client.Version_Update_Name(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
