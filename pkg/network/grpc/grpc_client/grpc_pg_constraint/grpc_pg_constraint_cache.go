...
func (crud Crud_GRPC) ReadFromCache(Oid int64, VersionID int64) (pg_constraint.PgConstraint, error) {
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgConstraint_ReadFromCache(Request)
	} else {
		Response, err = grpc_client_func.Client.PgConstraint_ReadFromCache(ctx, Request)
	}
	if err != nil {
