...
func (crud Crud_GRPC) ReadObject(m *object_postgres_migrate_pg_attribute.ObjectPostgresMigratePgAttribute) error {
	var err error
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PostgresMigratePgAttribute_ReadObject(Request)
	} else {
		Response, err = grpc_client_func.Client.PostgresMigratePgAttribute_ReadObject(ctx, Request)
	}
	if err != nil {
