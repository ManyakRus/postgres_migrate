...
func (crud Crud_GRPC) GetVersionModel() uint32 {
	...
	if VersionModel == 0 {
		VersionModel = pg_description.PgDescription{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_GRPC) Read(m *pg_description.PgDescription) error {
	var err error
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgDescription_Read(Request)
	} else {
		Response, err = grpc_client_func.Client.PgDescription_Read(ctx, Request)
	}
	if err != nil {
...
func (crud Crud_GRPC) Create(m *pg_description.PgDescription) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgDescription_Create(Request)
	} else {
		Response, err = grpc_client_func.Client.PgDescription_Create(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
...
func (crud Crud_GRPC) Update(m *pg_description.PgDescription) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgDescription_Update(Request)
	} else {
		Response, err = grpc_client_func.Client.PgDescription_Update(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
...
func (crud Crud_GRPC) Save(m *pg_description.PgDescription) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgDescription_Save(Request)
	} else {
		Response, err = grpc_client_func.Client.PgDescription_Save(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
