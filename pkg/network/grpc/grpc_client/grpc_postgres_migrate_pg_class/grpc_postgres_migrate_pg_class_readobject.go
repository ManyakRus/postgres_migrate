...
func (crud Crud_GRPC) ReadObject(m *object_postgres_migrate_pg_class.ObjectPostgresMigratePgClass) error {
	var err error
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PostgresMigratePgClass_ReadObject(Request)
	} else {
		Response, err = grpc_client_func.Client.PostgresMigratePgClass_ReadObject(ctx, Request)
	}
	if err != nil {
