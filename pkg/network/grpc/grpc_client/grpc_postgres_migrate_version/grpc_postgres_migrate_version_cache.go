...
func (crud Crud_GRPC) ReadFromCache(ID int64) (postgres_migrate_version.PostgresMigrateVersion, error) {
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PostgresMigrateVersion_ReadFromCache(Request)
	} else {
		Response, err = grpc_client_func.Client.PostgresMigrateVersion_ReadFromCache(ctx, Request)
	}
	if err != nil {
