...
func (crud Crud_GRPC) ReadObject(m *object_postgres_migrate_version.ObjectPostgresMigrateVersion) error {
	var err error
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PostgresMigrateVersion_ReadObject(Request)
	} else {
		Response, err = grpc_client_func.Client.PostgresMigrateVersion_ReadObject(ctx, Request)
	}
	if err != nil {
