...
package grpc_postgres_migrate_version
...
func (crud Crud_GRPC) GetVersionModel() uint32 {
	...
	if VersionModel == 0 {
		VersionModel = postgres_migrate_version.PostgresMigrateVersion{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_GRPC) Read(m *postgres_migrate_version.PostgresMigrateVersion) error {
	var err error
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PostgresMigrateVersion_Read(Request)
	} else {
		Response, err = grpc_client_func.Client.PostgresMigrateVersion_Read(ctx, Request)
	}
	if err != nil {
...
func (crud Crud_GRPC) Create(m *postgres_migrate_version.PostgresMigrateVersion) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PostgresMigrateVersion_Create(Request)
	} else {
		Response, err = grpc_client_func.Client.PostgresMigrateVersion_Create(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
...
func (crud Crud_GRPC) Update(m *postgres_migrate_version.PostgresMigrateVersion) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PostgresMigrateVersion_Update(Request)
	} else {
		Response, err = grpc_client_func.Client.PostgresMigrateVersion_Update(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
...
func (crud Crud_GRPC) Save(m *postgres_migrate_version.PostgresMigrateVersion) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PostgresMigrateVersion_Save(Request)
	} else {
		Response, err = grpc_client_func.Client.PostgresMigrateVersion_Save(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
