...
func (crud Crud_GRPC) ReadFromCache(Attname string, Attrelid int64, VersionID int64) (pg_attribute.PgAttribute, error) {
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgAttribute_ReadFromCache(Request)
	} else {
		Response, err = grpc_client_func.Client.PgAttribute_ReadFromCache(ctx, Request)
	}
	if err != nil {
