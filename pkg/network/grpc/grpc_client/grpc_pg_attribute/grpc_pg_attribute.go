...
func (crud Crud_GRPC) GetVersionModel() uint32 {
	...
	if VersionModel == 0 {
		VersionModel = pg_attribute.PgAttribute{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_GRPC) Read(m *pg_attribute.PgAttribute) error {
	var err error
	...
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgAttribute_Read(Request)
	} else {
		Response, err = grpc_client_func.Client.PgAttribute_Read(ctx, Request)
	}
	if err != nil {
...
func (crud Crud_GRPC) Create(m *pg_attribute.PgAttribute) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgAttribute_Create(Request)
	} else {
		Response, err = grpc_client_func.Client.PgAttribute_Create(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
...
func (crud Crud_GRPC) Update(m *pg_attribute.PgAttribute) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgAttribute_Update(Request)
	} else {
		Response, err = grpc_client_func.Client.PgAttribute_Update(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
...
func (crud Crud_GRPC) Save(m *pg_attribute.PgAttribute) error {
	var err error
	...
	var Response *grpc_proto.Response
	if grpc_nrpc.NeedNRPC == true {
		//Response, err = nrpc_client.Client.PgAttribute_Save(Request)
	} else {
		Response, err = grpc_client_func.Client.PgAttribute_Save(ctx, Request)
	}
	if err != nil {
		if grpc_client_func.IsErrorModelVersion(err) == true {
			log.Panic(err)
