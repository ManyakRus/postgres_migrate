...
// cache - кэш с данными
var cache *expirable.LRU[string, pg_class.PgClass]
...
func init() {
	cache = expirable.NewLRU[string, pg_class.PgClass](CACHE_SIZE, nil, time.Minute*CACHE_EXPIRE_MINUTES)
}

// ReadFromCache - находит запись в кеше или в БД по ID
func (crud Crud_DB) ReadFromCache(Oid int64, VersionID int64) (pg_class.PgClass, error) {
	var Otvet pg_class.PgClass
	var err error
...
func ReadFromCache_ctx(ctx context.Context, db *gorm.DB, Oid int64, VersionID int64) (pg_class.PgClass, error) {
	var Otvet pg_class.PgClass
	var err error

	// поищем сначала в кэше
	Identifier := (pg_class.StringIdentifier(Oid, VersionID))
