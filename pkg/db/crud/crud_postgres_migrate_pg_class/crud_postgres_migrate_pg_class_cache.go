...
func init() {
	cache = expirable.NewLRU[string, postgres_migrate_pg_class.PostgresMigratePgClass](CACHE_SIZE, nil, time.Minute*CACHE_EXPIRE_MINUTES)
}

// ReadFromCache - находит запись в кеше или в БД по ID
func (crud Crud_DB) ReadFromCache(Oid int64, VersionID int64) (postgres_migrate_pg_class.PostgresMigratePgClass, error) {
	var Otvet postgres_migrate_pg_class.PostgresMigratePgClass
	var err error
...
func ReadFromCache_ctx(ctx context.Context, db *gorm.DB, Oid int64, VersionID int64) (postgres_migrate_pg_class.PostgresMigratePgClass, error) {
	var Otvet postgres_migrate_pg_class.PostgresMigratePgClass
	var err error

	// поищем сначала в кэше
	Identifier := (postgres_migrate_pg_class.StringIdentifier(Oid, VersionID))
