...
const TableName string = "pg_index"
...
func (crud Crud_DB) Read(m *pg_index.PgIndex) error {
	var err error
...
func Read_ctx(ctx context.Context, db *gorm.DB, m *pg_index.PgIndex) error {
	var err error
...
func (crud Crud_DB) Save(m *pg_index.PgIndex) error {
	ctxMain := contextmain.GetContext()
	ctx, ctxCancelFunc := context.WithTimeout(ctxMain, time.Second*time.Duration(db_constants.TIMEOUT_DB_SECONDS))
	defer ctxCancelFunc()
...
func Save_ctx(ctx context.Context, db *gorm.DB, m *pg_index.PgIndex) error {
	var err error
	if micro.ContextDone(ctx) == true {
		err = context.Canceled
		return err
...
func (crud Crud_DB) Update(m *pg_index.PgIndex) error {
	ctxMain := contextmain.GetContext()
	ctx, ctxCancelFunc := context.WithTimeout(ctxMain, time.Second*time.Duration(db_constants.TIMEOUT_DB_SECONDS))
	defer ctxCancelFunc()
...
func Update_ctx(ctx context.Context, db *gorm.DB, m *pg_index.PgIndex) error {
	var err error
	if micro.ContextDone(ctx) == true {
		err = context.Canceled
		return err
...
func (crud Crud_DB) Create(m *pg_index.PgIndex) error {
	ctxMain := contextmain.GetContext()
	ctx, ctxCancelFunc := context.WithTimeout(ctxMain, time.Second*time.Duration(db_constants.TIMEOUT_DB_SECONDS))
	defer ctxCancelFunc()
...
func Create_ctx(ctx context.Context, db *gorm.DB, m *pg_index.PgIndex) error {
	var err error
	if micro.ContextDone(ctx) == true {
		err = context.Canceled
		return err
...
func (crud Crud_DB) create_update(m *pg_index.PgIndex) error {
	var err error
...
func create_update_ctx(ctx context.Context, db *gorm.DB, m *pg_index.PgIndex, MassNeedUpdateFields []string) error {
	var err error
