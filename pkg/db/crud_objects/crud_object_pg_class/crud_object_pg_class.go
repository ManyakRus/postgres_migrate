...
func (crud Crud_DB) ReadObject(m *object_pg_class.ObjectPgClass) error {
	var err error
...
func ReadObject_ctx(ctx context.Context, db *gorm.DB, m *object_pg_class.ObjectPgClass) error {
	var err error
	...
	//заполним model
	Model := m.PgClass
	err = crud_pg_class.Read_ctx(ctx, db, &Model)
	if err != nil {
