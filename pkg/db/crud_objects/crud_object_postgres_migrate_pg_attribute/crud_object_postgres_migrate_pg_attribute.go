...
package crud_object_postgres_migrate_pg_attribute
...
func (crud Crud_DB) ReadObject(m *object_postgres_migrate_pg_attribute.ObjectPostgresMigratePgAttribute) error {
	var err error
...
func ReadObject_ctx(ctx context.Context, db *gorm.DB, m *object_postgres_migrate_pg_attribute.ObjectPostgresMigratePgAttribute) error {
	var err error
	...
	//заполним model
	Model := m.PostgresMigratePgAttribute
	err = crud_postgres_migrate_pg_attribute.Read_ctx(ctx, db, &Model)
	if err != nil {
		err = fmt.Errorf(m.TableNameDB()+" Read() Attname: %v, Attrelid: %v, VersionID: %v, error: %w", m.Attname, m.Attrelid, m.VersionID, err)
	}
	m.PostgresMigratePgAttribute = Model
