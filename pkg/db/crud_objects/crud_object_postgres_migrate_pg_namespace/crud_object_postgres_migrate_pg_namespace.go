...
package crud_object_postgres_migrate_pg_namespace
...
func (crud Crud_DB) ReadObject(m *object_postgres_migrate_pg_namespace.ObjectPostgresMigratePgNamespace) error {
	var err error
...
func ReadObject_ctx(ctx context.Context, db *gorm.DB, m *object_postgres_migrate_pg_namespace.ObjectPostgresMigratePgNamespace) error {
	var err error
	...
	//заполним model
	Model := m.PostgresMigratePgNamespace
	err = crud_postgres_migrate_pg_namespace.Read_ctx(ctx, db, &Model)
	if err != nil {
		err = fmt.Errorf(m.TableNameDB()+" Read() Oid: %v, VersionID: %v, error: %w", m.Oid, m.VersionID, err)
	}
	m.PostgresMigratePgNamespace = Model
