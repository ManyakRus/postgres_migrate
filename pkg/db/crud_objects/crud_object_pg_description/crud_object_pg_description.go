...
package crud_object_pg_description
...
func (crud Crud_DB) ReadObject(m *object_pg_description.ObjectPgDescription) error {
	var err error
...
func ReadObject_ctx(ctx context.Context, db *gorm.DB, m *object_pg_description.ObjectPgDescription) error {
	var err error
	...
	//заполним model
	Model := m.PgDescription
	err = crud_pg_description.Read_ctx(ctx, db, &Model)
	if err != nil {
