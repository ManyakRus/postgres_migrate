...
package crud_object_postgres_migrate_pg_sequence
...
func (crud Crud_DB) ReadObject(m *object_postgres_migrate_pg_sequence.ObjectPostgresMigratePgSequence) error {
	var err error
...
func ReadObject_ctx(ctx context.Context, db *gorm.DB, m *object_postgres_migrate_pg_sequence.ObjectPostgresMigratePgSequence) error {
	var err error
	...
	//заполним model
	Model := m.PostgresMigratePgSequence
	err = crud_postgres_migrate_pg_sequence.Read_ctx(ctx, db, &Model)
	if err != nil {
		err = fmt.Errorf(m.TableNameDB()+" Read() Seqrelid: %v, VersionID: %v, error: %w", m.Seqrelid, m.VersionID, err)
	}
	m.PostgresMigratePgSequence = Model
	...
	if err != nil {
		err = fmt.Errorf(Version.TableNameDB()+" Read() Seqrelid: %v, VersionID: %v, error: %w", m.Seqrelid, m.VersionID, err)
	}
	m.Version = Version
