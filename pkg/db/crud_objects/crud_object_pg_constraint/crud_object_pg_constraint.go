...
package crud_object_pg_constraint
...
func (crud Crud_DB) ReadObject(m *object_pg_constraint.ObjectPgConstraint) error {
	var err error
...
func ReadObject_ctx(ctx context.Context, db *gorm.DB, m *object_pg_constraint.ObjectPgConstraint) error {
	var err error
	...
	//заполним model
	Model := m.PgConstraint
	err = crud_pg_constraint.Read_ctx(ctx, db, &Model)
	if err != nil {
		err = fmt.Errorf(m.TableNameDB()+" Read() Oid: %v, VersionID: %v, error: %w", m.Oid, m.VersionID, err)
	}
	m.PgConstraint = Model
