...
type Table_PostgresMigratePgAttribute struct {
	...
	Attmissingval string	`json:"attmissingval" gorm:"column:attmissingval;default:null"`	//В данном столбце размещается массив с одним элементом. Значение в этом элементе используется, когда столбец полностью отсутствует в строке, что имеет место, когда столбец добавляется с неизменчивым значением DEFAULT после создания строки. Это значение используется, только когда atthasmissing равен true. Если значение отсутствует, столбец будет содержать NULL.
