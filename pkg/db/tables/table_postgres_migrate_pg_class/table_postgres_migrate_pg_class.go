package table_postgres_migrate_pg_class


// Table_PostgresMigratePgClass - модель для таблицы postgres_migrate_pg_class: В каталоге postgres_migrate_pg_class описываются таблицы и практически всё, что имеет столбцы или каким-то образом подобно таблице. Сюда входят индексы (но смотрите также postgres_migrate_pg_index), последовательности, представления, материализованные представления, составные типы и таблицы TOAST; см. relkind. Далее, подразумевая все эти типы объектов, мы будем говорить об «отношениях». Не все столбцы здесь имеют смысл для всех типов отношений.
type Table_PostgresMigratePgClass struct {
	...
	Relchecks int32	`json:"relchecks" gorm:"column:relchecks;default:0"`	//Число ограничений CHECK в таблице; см. каталог postgres_migrate_pg_constraint
	...
	Relfrozenxid int64	`json:"relfrozenxid" gorm:"column:relfrozenxid;default:0"`	//Идентификаторы транзакций, предшествующие данному, в этой таблице заменены постоянным («замороженным») идентификатором транзакции. Это нужно для определения, когда требуется очищать таблицу для предотвращения зацикливания идентификаторов или для сокращения объёма pg_clog. Если это отношение — не таблица, значение равно нулю (InvalidTransactionId).
	...
	Relispartition bool	`json:"relispartition" gorm:"column:relispartition"`	//True, если таблица или индекс является секцией
	...
	Relnatts int32	`json:"relnatts" gorm:"column:relnatts;default:0"`	//Число пользовательских столбцов в отношении (системные столбцы не считаются). Столько же соответствующих строк должно быть в postgres_migrate_pg_attribute. См. также postgres_migrate_pg_attribute.attnum.
