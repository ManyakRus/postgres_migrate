package table_pg_index


// Table_PgIndex - модель для таблицы pg_index: В каталоге pg_index содержится часть информации об индексах. Остальная информация в основном находится в pg_class.
type Table_PgIndex struct {
	Indcheckxmin bool	`json:"indcheckxmin" gorm:"column:indcheckxmin"`	//Если true, запросы не должны использовать этот индекс, пока поле xmin данной записи в pg_index не окажется ниже их горизонта событий TransactionXmin, так как таблица может содержать оборванные цепочки HOT, с видимыми несовместимыми строками
	...
	Indexrelid int64	`json:"indexrelid" gorm:"column:indexrelid;primaryKey;autoIncrement:true"`	//OID записи в pg_class для этого индекса
	...
	Indnatts int32	`json:"indnatts" gorm:"column:indnatts;default:0"`	//Число столбцов в индексе (повторяет значение pg_class.relnatts)
	...
	Indrelid int64	`json:"indrelid" gorm:"column:indrelid;primaryKey;autoIncrement:true"`	//OID записи в pg_class для таблицы, к которой относится этот индекс
