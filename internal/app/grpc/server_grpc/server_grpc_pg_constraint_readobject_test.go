...
func Test_server_PgConstraint_ReadObject(t *testing.T) {
	...
	Request.VersionModel = object_pg_constraint.ObjectPgConstraint{}.GetStructVersion()

	server1 := &ServerGRPC{}
	_, err := server1.PgConstraint_ReadObject(ctx, &Request)
	if err != nil && crud_func.IsRecordNotFound(err) == false {
		t.Error("Test_server_PgConstraint_ReadObject() error: ", err)
	}
}
