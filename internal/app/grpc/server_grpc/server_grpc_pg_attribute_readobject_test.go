...
func Test_server_PgAttribute_ReadObject(t *testing.T) {
	...
	Request.VersionModel = object_pg_attribute.ObjectPgAttribute{}.GetStructVersion()

	server1 := &ServerGRPC{}
	_, err := server1.PgAttribute_ReadObject(ctx, &Request)
	if err != nil && crud_func.IsRecordNotFound(err) == false {
		t.Error("Test_server_PgAttribute_ReadObject() error: ", err)
	}
}
