...
func Test_server_PostgresMigratePgSequence_ReadObject(t *testing.T) {
	...
	Request.VersionModel = object_postgres_migrate_pg_sequence.ObjectPostgresMigratePgSequence{}.GetStructVersion()

	server1 := &ServerGRPC{}
	_, err := server1.PostgresMigratePgSequence_ReadObject(ctx, &Request)
	if err != nil && crud_func.IsRecordNotFound(err) == false {
		t.Error("Test_server_PostgresMigratePgSequence_ReadObject() error: ", err)
	}
}
