...
func Test_server_PgClass_ReadObject(t *testing.T) {
	...
	Request.VersionModel = object_pg_class.ObjectPgClass{}.GetStructVersion()

	server1 := &ServerGRPC{}
	_, err := server1.PgClass_ReadObject(ctx, &Request)
	if err != nil && crud_func.IsRecordNotFound(err) == false {
		t.Error("Test_server_PgClass_ReadObject() error: ", err)
	}
}
