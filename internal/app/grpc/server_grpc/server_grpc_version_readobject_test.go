...
func Test_server_Version_ReadObject(t *testing.T) {
	...
	Request.VersionModel = object_version.ObjectVersion{}.GetStructVersion()

	server1 := &ServerGRPC{}
	_, err := server1.Version_ReadObject(ctx, &Request)
	if err != nil && crud_func.IsRecordNotFound(err) == false {
		t.Error("Test_server_Version_ReadObject() error: ", err)
	}
}
