...
func Test_server_PgAttribute_ReadFromCache(t *testing.T) {
	...
	server1 := &ServerGRPC{}
	Otvet, err := server1.PgAttribute_ReadFromCache(ctx, &Request)
	if err != nil {
		t.Error("Test_server_PgAttribute_ReadFromCache() error: ", err)
	}
	if Otvet.ModelString == "" {
		t.Error("Test_server_PgAttribute_ReadFromCache() error: ModelString=''")
	}
}
