...
// PgAttribute_ReadFromCache - читает и возвращает модель из кеша или БД
func (s *ServerGRPC) PgAttribute_ReadFromCache(ctx context.Context, Request *grpc_proto.Request_String_Int64_Int64) (*grpc_proto.Response, error) {
	...
	Model := pg_attribute.PgAttribute{}
	Model, err = crud_pg_attribute.ReadFromCache_ctx(ctx, db, Attname, Attrelid, VersionID)
	if err != nil {
		return &Otvet, err
	}
