...
func Test_server_PgNamespace_ReadFromCache(t *testing.T) {
	...
	server1 := &ServerGRPC{}
	Otvet, err := server1.PgNamespace_ReadFromCache(ctx, &Request)
	if err != nil {
		t.Error("Test_server_PgNamespace_ReadFromCache() error: ", err)
	}
	if Otvet.ModelString == "" {
		t.Error("Test_server_PgNamespace_ReadFromCache() error: ModelString=''")
	}
}
