...
func Test_server_PostgresMigratePgClass_ReadObject(t *testing.T) {
	config_main.LoadEnv()
	...
	Request.VersionModel = object_postgres_migrate_pg_class.ObjectPostgresMigratePgClass{}.GetStructVersion()

	server1 := &ServerGRPC{}
	_, err := server1.PostgresMigratePgClass_ReadObject(ctx, &Request)
	if err != nil && crud_func.IsRecordNotFound(err) == false {
		t.Error("Test_server_PostgresMigratePgClass_ReadObject() error: ", err)
	}
}
