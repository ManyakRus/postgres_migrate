...
func Test_server_PgConstraint_ReadFromCache(t *testing.T) {
	...
	server1 := &ServerGRPC{}
	Otvet, err := server1.PgConstraint_ReadFromCache(ctx, &Request)
	if err != nil {
		t.Error("Test_server_PgConstraint_ReadFromCache() error: ", err)
	}
	if Otvet.ModelString == "" {
		t.Error("Test_server_PgConstraint_ReadFromCache() error: ModelString=''")
	}
}
