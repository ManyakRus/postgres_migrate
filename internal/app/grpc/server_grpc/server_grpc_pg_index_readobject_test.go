...
func Test_server_PgIndex_ReadObject(t *testing.T) {
	...
	Request.VersionModel = object_pg_index.ObjectPgIndex{}.GetStructVersion()

	server1 := &ServerGRPC{}
	_, err := server1.PgIndex_ReadObject(ctx, &Request)
	if err != nil && crud_func.IsRecordNotFound(err) == false {
		t.Error("Test_server_PgIndex_ReadObject() error: ", err)
	}
}
