...
func Start_Tables_alter(Settings *config.SettingsINI, VersionID int64) (string, error) {
	var err error
	Otvet := ""

	//найдём массив новых
	MassClass, err := Find_MassClass_Alter(Settings, VersionID)
	if err != nil {
...
// Find_MassClass_Alter - возвращает массив postgres_migrate_pg_class
func Find_MassClass_Alter(Settings *config.SettingsINI, VersionID int64) ([]TableNameAlter, error) {
	Otvet := make([]TableNameAlter, 0)
	var err error
...
func TextSQL_Alter(Settings *config.SettingsINI, MassNames []TableNameAlter) (string, error) {
	Otvet := ""
	var err error
